...
function renderTenants(tenants) {
    ...
    tenants.forEach(t => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <button onclick="openExtendModal('${(t.id || t._id)}')" class="btn btn-success btn-sm">Activate/Extend</button>
                <button onclick="resetPassword('${(t.id || t._id)}')" class="btn btn-info btn-sm" style="background-color: #17a2b8; color: white;">Reset Pass</button>
                ${t.status === 'active'
                ? `<button onclick="toggleHold('${(t.id || t._id)}', 'on_hold')" class="btn btn-warning btn-sm">Hold</button>`
                : `<button onclick="toggleHold('${(t.id || t._id)}', 'active')" class="btn btn-primary btn-sm">Unhold</button>`
            }
                <button onclick="terminateTenant('${(t.id || t._id)}')" class="btn btn-danger btn-sm">Terminate</button>
            </td>
        `;
